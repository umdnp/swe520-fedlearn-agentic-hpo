...
In healthcare, data are typically distributed across institutions and cannot be pooled for centralized analysis.
Under these constraints, hyperparameter optimization (HPO) in federated learning cannot rely on direct data
...
This experiment uses the eICU Collaborative Research Database (eICU-CRD), a multi-center critical care dataset
reflecting real-world healthcare settings in which federated learning may be necessary. The emphasis is not on
maximizing benchmark accuracy, but on understanding whether agent-driven HPO provides a practical and structurally
sound approach under real-world federated constraints.
